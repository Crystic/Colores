package net.crystic.colores.tileentity;
...
public class TileEntityRainbowExtractor extends TileEntity implements
		ISidedInventory {
	...
	private static int getItemInput(ItemStack itemstack) {
		if (itemstack == null) {
			...
			if (item == Colores.itm_BlackFossil)
				return 1;
			if (item == Colores.itm_BrownFossil)
				return 1;
			if (item == Colores.itm_GreenFossil)
				return 1;
			if (item == Colores.itm_RedFossil)
				return 1;
			if (item == Colores.itm_WhiteFossil)
				return 1;
			if (item == Colores.itm_YellowFossil)
				return 1;
	...
	public void updateEntity() {
		...
		if (!this.worldObj.isRemote) {
			...
			if (flag != this.isBurning()) {
				flag1 = true;
				RainbowExtractor.updateRainbowExtractorBlockState(
	...
	public boolean canSmelt() {
		if (this.slots[0] == null) {
			...
			ItemStack itemstack = ExtractorRecipes.extraction(slots[0]
					.getItem());
